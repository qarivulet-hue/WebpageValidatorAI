package WebpageValidatorAIPKG.CNVT_testNG;
...
import WebpageValidatorAIPKG.DiffHTML;
...
public class WebQATest {
    ...
    public void testQAAudit(String url) throws Exception {
        WebsiteData websiteData = performQAAudit(url);
        websiteDataList.add(websiteData);
        generateDashboardHTML();

        // Assertions for SEO score and critical elements
        int seoScore = calculateSeoScore(websiteData);
        Assert.assertTrue(seoScore >= 0 && seoScore <= 100, "SEO score is out of valid range: " + seoScore);
        Assert.assertFalse(websiteData.title.equals("No data found"), "Page title was not retrieved successfully");
        int h1Count = websiteData.headers.getOrDefault("h1", Collections.emptyList()).size();
        Assert.assertEquals(h1Count, 1, "Expected exactly one H1 tag, found: " + h1Count);
    }
    ...
    public Object[][] provideUrls() {
        return new Object[][] {
            { "https://havis.com/" }
            // Add more URLs here for testing multiple websites
        };
    }

    private WebsiteData performQAAudit(String url) throws Exception {
        try {
            driver.get(url);
            ...
            return new WebsiteData(url, "No data found", "No data found", new HashMap<>(),
        ...
        String visibleText = "No data found";
        try {
            WebElement body = driver.findElement(By.tagName("body"));
            visibleText = body.getText() != null ? body.getText() : "No data found";
        } catch (NoSuchElementException ex) {
            LOGGER.warning("Body text not found for URL: " + url);
        } catch (Exception e) {
            LOGGER.warning("Error retrieving body text for URL: " + url + ". Error: " + e.getMessage());
        }
        List<String> textChunks = Arrays.asList(visibleText.split("\\n+"));
        List<GrammarIssue> grammarIssues = parallelGrammarCheck(textChunks);

        List<BrokenLink> brokenLinks = parallelBrokenLinkCheck(driver.findElements(By.tagName("a")), exactUrl);
        return new WebsiteData(exactUrl, title, metaDescription, headers, buttons, paragraphs,
        ...
        for (WebElement link : links) {
            ...
            if (linkUrl != null && !linkUrl.isEmpty()) {
                try {
                    ...
                    URL absoluteUrl = new URL(baseUrl, linkUrl);
    ...
    private List<GrammarIssue> grammarCheck(String text) {
        ...
        try {
            ...
            LOGGER.warning("Grammar check failed for text: " + text + ". Error: " + e.getMessage());
        }
        return issues;
    }
        ...
        while (retries > 0) {
            try {
                ...
                if (status == 200) {
                    ...
                    if (content.contains("<span>404</span>") || content.contains("404") || content.contains("Page Not Found")) {
                        LOGGER.info("Soft 404 detected for URL: " + urlStr);
                        return 404;
                    }
                } else if (status >= 400 && status < 500) {
                    LOGGER.info("Client error detected for URL: " + urlStr + ", Status: " + status);
                    return status;
                } else if (status >= 500) {
                    LOGGER.info("Server error detected for URL: " + urlStr + ", Status: " + status);
                    return status;
                }
...
.length() > 160) {
    ...
    private void generateDashboardHTML() throws IOException {
        ...
        sb.append(".seo-card .ok { color: #4caf50; font-weight: 700; personally-align: middle; margin-left:8px; }\n");
...
sb.append(".heading-detail.active { display: block; }\n");
        ...
        sb.append(".issues-subtabs button { flex: 1; padding: 14px 0; background: #f9fafb; border: none; cursor: pointer; font-weight: 600; font-size Motif; font-size: 14px; color: #e07a5f; transition: all 0.3s ease; }\n");
        ...
        sb.append("<button class=\"issues-tab active\" role=\"tab\" aria-selected=\"true\" tabindex=\"0\" data-tab=\"grammar-issues\">Grammar Issues (")
        ...
        sb.append("<h3>⚠ Grammar and Spelling Issues</h3>\n");
        if (data.grammarIssues.isEmpty()) {
            sb.append("<p>No grammar issues found.</p>\n");
        } else {
            for (GrammarIssue g : data.grammarIssues) {
                sb.append("<strong>Issue detected:</strong>\n");
                sb.append("<textarea readonly rows=\"2\">").append(escapeHtml(g.context)).append("</textarea>\n");
                sb.append("<strong>Suggested correction:</strong>\n");
                sb.append("<input type=\"text\" readonly value=\"").append(escapeHtml(g.suggestion)).append("\" />\n");
                sb.append("<strong>Issue description:</strong>\n");
                sb.append("<textarea readonly rows=\"2\">").append(escapeHtml(g.message)).append("</textarea>\n");
            }
        }
        sb.append("</div>\n");
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("WebQATEST.html"))) {
	...
	public static void main(String[] args) throws Exception {
		muteLogs();
		DiffHTML qa = new DiffHTML();
		qa.runQAAudit("https://havis.com/");
	}
}	