...
public class DiffHTML {
    ...
    private static final Logger LOGGER = Logger.getLogger(DiffHTML.class.getName());
    ...
    public void runQAAudit(String url) throws Exception {
        ...
        LOGGER.info("Extracted " + allTextContent.size() + " text elements for grammar checking");
    ...
    private List<GrammarIssue> grammarCheck(String text) {
        ...
        try {
            ...
            for (RuleMatch match : matches) {
                ...
                String detailedMessage = match.getMessage() + " in " + elementType;
                
                issues.add(new GrammarIssue(context, suggestion, detailedMessage, elementType, actualText));
                LOGGER.info("Grammar issue found in " + elementType + ": " + context + " -> " + suggestion);
            }
        } catch (Throwable e) {
    ...
    private void generateDashboardHTML() throws IOException {
        ...
        if (data.grammarIssues.isEmpty()) {
            ...
            for (GrammarIssue g : data.grammarIssues) {
                ...
                sb.append("<span style=\"background: #e07a5f; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px; font-weight: bold;\">GRAMMAR ERROR</span>\n");
                sb.append("</div>\n");
